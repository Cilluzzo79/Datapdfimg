...
async def validate_file(file: UploadFile) -> Tuple[str, str]:
    ...
    if not is_allowed_file(filename):
        ...
        raise HTTPException(
            status_code=400, 
            detail=f"Tipo di file non supportato. Tipi supportati: {settings.ALLOWED_IMAGE_EXTENSIONS + settings.ALLOWED_PDF_EXTENSIONS + settings.ALLOWED_EXCEL_EXTENSIONS + settings.ALLOWED_CSV_EXTENSIONS}"
        )
